-- Migration: Create esocial_fatores_risco table for S-2240 risk factors
CREATE TABLE IF NOT EXISTS esocial_fatores_risco (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    cargo TEXT NOT NULL,
    descricao_atividades TEXT,
    codigo_fator_risco TEXT NOT NULL,
    descricao_fator_risco TEXT,
    epi_utilizado TEXT,
    created_at TIMESTAMPTZ DEFAULT now(),
    updated_at TIMESTAMPTZ DEFAULT now()
);

CREATE INDEX IF NOT EXISTS idx_esocial_fatores_risco_cargo ON esocial_fatores_risco(cargo);
CREATE INDEX IF NOT EXISTS idx_esocial_fatores_risco_codigo ON esocial_fatores_risco(codigo_fator_risco);

ALTER TABLE esocial_fatores_risco ENABLE ROW LEVEL SECURITY;

CREATE POLICY IF NOT EXISTS esocial_fatores_risco_select ON esocial_fatores_risco FOR SELECT TO authenticated
    USING (true);

CREATE POLICY IF NOT EXISTS esocial_fatores_risco_admin ON esocial_fatores_risco FOR ALL TO authenticated
    USING (EXISTS (SELECT 1 FROM users_unified WHERE id = auth.uid() AND role = 'ADMIN'));

-- Seed data
INSERT INTO esocial_fatores_risco (cargo, descricao_atividades, codigo_fator_risco, descricao_fator_risco, epi_utilizado) VALUES
