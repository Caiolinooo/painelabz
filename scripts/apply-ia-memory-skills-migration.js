/**
 * Apply IA Companion LTM + Skills migrations via service role / DB URL.
 * Does NOT print secrets.
 *
 * Usage: node scripts/apply-ia-memory-skills-migration.js
 */
const fs = require('fs');
...
const migrations = [
  'supabase/migrations/20260727_000001_ia_user_memory.sql',
  'supabase/migrations/20260727_000002_ia_user_skills.sql',
];
...
async function verifyTables(supabase) {
  const tables = ['ia_user_memory', 'ia_user_skills'];
  const results = {};
  for (const t of tables) {
    ...
    if (!error) {
      ...
      // table exists but maybe empty / RLS — still reachable
