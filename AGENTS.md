...
- **Viewport do portal**: shell `h-dvh` + coluna de conteúdo `flex flex-col min-h-0 flex-1`; página preenche; `<main>` com padding responsivo (`px-3 py-3 sm:px-4 sm:py-4 md:px-8 md:py-6`) e `-webkit-overflow-scrolling: touch;`. Em desktop (`lg:`), filtros/header `shrink-0` e lista/tabela primária em `flex-1 min-h-0 overflow-auto`; em mobile/telas compactas (`< 1024px`), `GtPageShell` permite rolagem vertical fluida (`overflow-y-auto lg:overflow-hidden`) e as tabelas/grades mantêm altura mínima (`min-h-[320px]` / `min-h-[360px]`), garantindo visibilidade e cliques em qualquer resolução. Menus mobile fecham automaticamente na navegação (`pathname`). Menu lateral (`MainLayout`): `<nav>` é o scrollport (`flex-1 min-h-0 overflow-y-auto`), header do logo e créditos `shrink-0` — sem `min-h-0` o menu empurra o rodapé para fora da viewport (conteúdo cortado e inalcançável). Estado colapsado/aberto vem do localStorage no mount: renderizar já no estado final com transições desabilitadas até ser pintado (`layoutTransition` / `isLayoutStateLoaded`, double-rAF) — sem animação de esticar ao abrir; transições apenas em toggles iniciados pelo usuário.
...
- `src/components/gestao-tripulantes/AGENTS.md` — lookups criáveis, KPIs ativos, viewport dia/semana, inbox ASO logística, scroll sticky da escala, `GtPageShell` das páginas GT-family
