        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="text-center py-12">
            ...
            <p className="mt-1 text-sm text-gray-500">
              {t('academy.facaLoginParaVerSuasNotificacoes')}
            </p>
          </div>
        </div>
      </MainLayout>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div className="flex space-x-1 bg-gray-100 p-1 rounded-lg">
                ...
                >
                  {t('academy.todas')} ({notifications.length})
                </button>
                <button
                ...
                >
                  {t('academy.naoLidas')} ({unreadCount})
                </button>
              </div>
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-sm text-gray-600">
                    {selectedNotifications.length > 0
                      ? `${selectedNotifications.length} ${selectedNotifications.length > 1 ? t('academy.selecionadas') : t('academy.selecionada')}`
                      : t('academy.selecionarTodas')
                    }
                  </span>
                </div>
              )}
            ...
            <div className="flex items-center space-x-2">
              ...
              {unreadCount > 0 && (
                ...
                >
                  <CheckIcon className="w-4 h-4 mr-1" />
                  {t('academy.marcarTodasComoLidas')}
                </button>
              )}
            </div>
          </div>
        ...
        {loading ? (
          ...
          <div className="space-y-2">
            {notifications.map((notification) => (
              <div
                key={notification.id}
                className={`bg-white rounded-lg shadow-sm border border-gray-200 p-4 hover:shadow-md transition-shadow ${!notification.is_read ? 'border-l-4 border-l-blue-500' : ''
                  }`}
              >
