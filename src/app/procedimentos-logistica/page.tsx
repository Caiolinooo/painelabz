...
export default function ProcedimentosLogisticaPage() {
  ...
  // Fetch documents from the API for the Logística category
  useEffect(() => {
    (async () => {
      try {
        setLoading(true);
        const res = await fetch('/api/documents?category=' + encodeURIComponent('Logística'));
        if (res.ok) {
          const data = await res.json();
          // Filter only enabled documents and sort by order
        ...
        console.warn('Falha ao carregar procedimentos de logística:', e);
      } finally {
        setLoading(false);
      }
    })();
  ...
  return (
    <MainLayout>
      <h1 className="text-3xl font-bold text-abz-text-black mb-6">{t('procedures.title', 'Procedimentos de Logística')}</h1>

      <div className="bg-white p-6 rounded-lg shadow-md">
        <h2 className="text-2xl font-semibold text-abz-text-black mb-6">{t('procedures.mainDocument', 'Documentos')}</h2>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center text-center text-gray-500 py-12 px-6 border-2 border-dashed border-gray-300 rounded-lg">
            <FiClipboard className="h-12 w-12 text-gray-400 mb-4" />
            <h3 className="text-xl font-semibold text-abz-text-black mb-2">
              {t('procedures.noDocuments', 'Nenhum documento disponível')}
            </h3>
            <p className="text-sm">
              {t('procedures.noDocumentsDescription', 'Os documentos de procedimentos de logística serão disponibilizados em breve.')}
            </p>
          </div>
        ) : (
              ...
              >
                ...
                <div className="flex items-center gap-2 mt-4">
                  ...
                  >
                    ...
                    {t('procedures.download', 'Download')}
                  </a>
                </div>
              </div>
            ))}
