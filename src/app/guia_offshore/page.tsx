...
export default function GuiaOffshorePage() {
    ...
    // Fetch documents from the API for the Guia Offshore category
    useEffect(() => {
        (async () => {
            try {
                setLoading(true);
                const res = await fetch('/api/documents?category=' + encodeURIComponent('Guia Offshore'));
                if (res.ok) {
                ...
                console.warn('Falha ao carregar documentos do Guia Offshore:', e);
            } finally {
                setLoading(false);
            }
        })();
    ...
    return (
        <MainLayout>
            <h1 className="text-3xl font-bold text-abz-text-black mb-6">
                {t('guiaOffshore.title', 'Guia do Colaborador Offshore')}
            </h1>

            <div className="bg-white p-6 rounded-lg shadow-md">
                <h2 className="text-2xl font-semibold text-abz-text-black mb-6">
                    {t('guiaOffshore.mainDocument', 'Documentos do Guia')}
                </h2>
                ...
                {loading ? (
                    ...
                    <div className="flex flex-col items-center justify-center text-center text-gray-500 py-12 px-6 border-2 border-dashed border-gray-300 rounded-lg">
                        ...
                        <h3 className="text-xl font-semibold text-abz-text-black mb-2">
                            {t('guiaOffshore.noDocuments', 'Nenhum documento disponível')}
                        </h3>
                        <p className="text-sm">
                            {t('guiaOffshore.noDocumentsDescription', 'Os documentos do guia do colaborador offshore serão disponibilizados em breve.')}
                        </p>
                        <p className="text-xs text-gray-400 mt-4">
                            {t('guiaOffshore.adminNote', 'Administradores podem adicionar documentos em Admin > Documentos')}
                        </p>
                    </div>
                ) : (
                            ...
                            >
                                ...
                                <div className="flex items-center gap-2 mt-4">
                                    ...
                                    >
                                        ...
                                        {t('guiaOffshore.view', 'Visualizar')}
                                    </button>
                                    <a
                                        href={doc.file}
                                        download
                                    ...
                                    >
                                        ...
                                        {t('guiaOffshore.download', 'Download')}
                                    </a>
                                </div>
                            </div>
                        ))}
