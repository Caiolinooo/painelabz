...
- **Workflow de Fechamento e Auditoria (`gt_relatorios_aprovacoes`)**:
  ...
  - `GET|PUT /api/gestao-tripulantes/relatorio-mensal/config`: dia de corte, e-mails DP/CC, aprovadores, auto-envio, templates. PUT só ADMIN/MANAGER. GET devolve `availableUsers` e `availableManagers` (mesmo array) via `listarUsuariosPortalAtivos()` (usuários ativos com e-mail, **sem filtro de role**). `listarGestoresPortal()` permanece para ASO logística.
