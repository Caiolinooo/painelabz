...
export async function GET(
  ...
  try {
    ...
    console.error('Erro ao obter embarcação:', error);
...
export async function PUT(
  ...
  try {
    ...
    if (error) {
      ...
      return NextResponse.json({ error: 'Erro ao atualizar embarcação' }, { status: 500 });
    }
  ...
  try {
    ...
    if (count && count > 0) {
      return NextResponse.json({
        error: `Não é possível excluir: ${count} colaborador(es) vinculado(s) a esta embarcação`
      }, { status: 409 });
    }
    ...
    if (error) {
      ...
      return NextResponse.json({ error: 'Erro ao excluir embarcação' }, { status: 500 });
    }

    return NextResponse.json({ success: true, message: 'Embarcação excluída com sucesso' });
  } catch (error) {
    console.error('Erro ao excluir embarcação:', error);
    return NextResponse.json({ error: 'Erro interno do servidor' }, { status: 500 });
  }
