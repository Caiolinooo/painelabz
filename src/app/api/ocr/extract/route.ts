...
export async function POST(request: NextRequest) {
  try {
    ...
    const tipoDocumento = (formData.get('tipo_documento') as string) || undefined;
    const bucketName = (formData.get('bucket') as string) || 'gestao-tripulantes-documentos';
    ...
    await supabaseAdmin.storage.from(bucketName).remove([tempPath]).catch(() => {});
