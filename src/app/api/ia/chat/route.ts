            ...
            try {
              const result = await chatCompletion(
                ...
                { 
                  ...
                  onStatus: (status) => {
                    controller.enqueue(encoder.encode(`data: ${***REMOVED*** status })}\n\n`));
                  }
                }, 
              ...
              controller.enqueue(encoder.encode(`data: ${***REMOVED*** content: finalContent, metadata: { dashboard } })}\n\n`));
              
              // Evento final compatível com o frontend
              const finalEvent = `data: ${***REMOVED*** done: true, fullContent: finalContent, metadata: { dashboard } })}\n\n`;
              ...
              controller.enqueue(encoder.encode(`data: ${***REMOVED*** content: `\n\n**Erro:** ${errMsg}` })}\n\n`));
              controller.enqueue(encoder.encode(`data: ${***REMOVED*** done: true })}\n\n`));
            } finally {
              controller.close();
            }
          }
