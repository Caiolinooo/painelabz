...
export default function ListaPresencaPage() {
    ...
    const fetchListas = useCallback(async () => {
        try {
            ...
            toast.error(t('presenca.error_loading', 'Erro ao carregar listas de presença'));
        } finally {
            setIsLoading(false);
        }
    ...
    const handleCloseList = async (id: string) => {
        if (!confirm(t('presenca.confirm_close', 'Deseja fechar esta lista? Não será possível adicionar mais assinaturas.'))) return;
        try {
            ...
            const data = await res.json();
            if (data.success) {
                toast.success(t('presenca.list_closed', 'Lista fechada'));
                fetchListas();
            }
        } catch {
            toast.error(t('presenca.error_closing', 'Erro ao fechar lista'));
        }
    };

    const handleDeleteList = async (id: string) => {
        if (!confirm(t('presenca.confirm_delete', 'Deseja excluir esta lista permanentemente?'))) return;
        try {
            const res = await fetchWithAuth(`/api/lista-presenca?id=${id}`, { method: 'DELETE' });
            const data = await res.json();
            if (data.success) {
                toast.success(t('presenca.list_deleted', 'Lista excluída'));
                fetchListas();
            }
        } catch {
            toast.error(t('presenca.error_deleting', 'Erro ao excluir lista'));
        }
    };

    const formatDate = (d: string) => new Date(d + 'T00:00:00').toLocaleDateString(locale, { day: '2-digit', month: '2-digit', year: 'numeric' });
        ...
        <MainLayout>
            <div className="max-w-7xl mx-auto space-y-6">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        <h1 className="text-2xl font-bold text-gray-900">{t('presenca.title', 'Lista de Presença')}</h1>
                        <p className="text-sm text-gray-500 mt-1">{t('presenca.subtitle', 'Controle de presença com assinatura digital')}</p>
                    </div>
                    {isManager && (
                        <button
                        ...
                        >
                            ...
                            {t('presenca.new_list', 'Nova Lista')}
                        </button>
                    )}
                </div>
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                    <div className="relative flex-1">
                        ...
                        <input
                            type="text"
                            placeholder={t('presenca.search_placeholder', 'Buscar lista...')}
                            value={searchTerm}
                    ...
                    >
                        <option value="">{t('presenca.filter.all_status', 'Todos os status')}</option>
                        <option value="aberta">{t('presenca.filter.open', 'Abertas')}</option>
                        <option value="fechada">{t('presenca.filter.closed', 'Fechadas')}</option>
                        <option value="cancelada">{t('presenca.filter.canceled', 'Canceladas')}</option>
                    </select>
                </div>
                ...
                {isLoading ? (
                    ...
                    <div className="text-center py-20 bg-white rounded-2xl border border-gray-100">
                        ...
                        <h3 className="text-lg font-medium text-gray-500">{t('presenca.no_list_found', 'Nenhuma lista encontrada')}</h3>
                        <p className="text-sm text-gray-400 mt-1">
                            {isManager ? t('presenca.create_first_prompt', 'Clique em "Nova Lista" para criar sua primeira lista.') : t('presenca.no_lists_available', 'Nenhuma lista de presença disponível.')}
                        </p>
                    </div>
                ) : (
                                ...
                                <div key={lista.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow overflow-hidden">
                                    ...
                                    <div className="p-5 pb-3">
                                        <div className="flex items-start justify-between gap-2">
                                            ...
                                            <span className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium whitespace-nowrap ${statusCfg.color}`}>
                                                ...
                                                {t(`presenca.status.${lista.status}`, statusCfg.label)}
                                            </span>
                                        </div>
                                        ...
                                        <div className="mt-3 space-y-1.5 text-xs text-gray-500">
                                            ...
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span>
                                                    {lista.total_participantes} {t(`presenca.participant_${lista.total_participantes === 1 ? 'single' : 'plural'}`, lista.total_participantes === 1 ? 'participante' : 'participantes')}
                                    ...
                                    <div className="px-5 py-3 bg-gray-50/50 border-t border-gray-100 flex items-center justify-between gap-2">
                                        <button
                                            ...
                                            title={t('presenca.copy_public_link', 'Copiar link público')}
                                        >
                                        ...
                                        <div className="flex items-center gap-1">
                                            <a
                                                ...
                                                title={t('presenca.view_details', 'Ver detalhes')}
                                            >
                                            ...
                                            {isManager && lista.status === 'aberta' && (
                                                <button
                                                    ...
                                                    title={t('presenca.close_list', 'Fechar lista')}
                                                >
...
function CreateListaModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!form.titulo || !form.data_evento) {
            toast.error(t('presenca.form.validation_required', 'Preencha título e data do evento'));
            return;
        }
        ...
        try {
            ...
            const data = await res.json();
            if (data.success) {
                toast.success(t('presenca.form.success_created', 'Lista criada com sucesso!'));
                onCreated();
            } else {
                toast.error(data.error || t('presenca.form.error_creating', 'Erro ao criar lista'));
            }
        } catch {
            toast.error(t('presenca.form.error_creating', 'Erro ao criar lista'));
        } finally {
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-2xl max-w-lg w-full shadow-2xl max-h-[90vh] overflow-y-auto">
                <div className="flex justify-between items-center px-6 py-4 border-b border-gray-100 sticky top-0 bg-white rounded-t-2xl">
                    <h3 className="text-lg font-semibold text-gray-900">{t('presenca.new_list_title', 'Nova Lista de Presença')}</h3>
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                    <div>
                        <label className="block text-xs font-medium text-gray-600 mb-1.5">{t('presenca.form.title_label', 'Título')} *</label>
                        <input
                            type="text"
                            required
                            value={form.titulo}
                            onChange={(e) => setForm(f => ({ ...f, titulo: e.target.value }))}
                            placeholder={t('presenca.form.title_placeholder', 'Ex: Reunião de Segurança — Novembro 2025')}
                    ...
                    <div className="grid grid-cols-3 gap-3">
                        <div>
                            <label className="block text-xs font-medium text-gray-600 mb-1.5">{t('presenca.form.date_label', 'Data')} *</label>
                            <input type="date" required value={form.data_evento} onChange={(e) => setForm(f => ({ ...f, data_evento: e.target.value }))}
                                className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-100 focus:border-blue-400 outline-none" />
                        </div>
                        <div>
                            <label className="block text-xs font-medium text-gray-600 mb-1.5">{t('presenca.form.start_label', 'Início')}</label>
                            <input type="time" value={form.hora_inicio} onChange={(e) => setForm(f => ({ ...f, hora_inicio: e.target.value }))}
                                className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-100 focus:border-blue-400 outline-none" />
                        </div>
                        <div>
                            <label className="block text-xs font-medium text-gray-600 mb-1.5">{t('presenca.form.end_label', 'Fim')}</label>
                    ...
                    <div>
                        <label className="block text-xs font-medium text-gray-600 mb-1.5">{t('presenca.form.location_label', 'Local')}</label>
                        <input type="text" value={form.local} onChange={(e) => setForm(f => ({ ...f, local: e.target.value }))}
                            placeholder={t('presenca.form.location_placeholder', 'Ex: Sala de Reuniões 3 — Sede RJ')}
                            className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-100 focus:border-blue-400 outline-none" />
                    </div>

                    <div>
                        <label className="block text-xs font-medium text-gray-600 mb-1.5">{t('presenca.form.agenda_label', 'Pauta / Conteúdo')}</label>
                        <textarea value={form.pauta} onChange={(e) => setForm(f => ({ ...f, pauta: e.target.value }))}
                            rows={3} placeholder={t('presenca.form.agenda_placeholder', 'Descreva a pauta ou conteúdo do evento...')}
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <div>
                            <label className="block text-xs font-medium text-gray-600 mb-1.5">{t('presenca.form.max_participants_label', 'Máx. Participantes')}</label>
                            <input type="number" min="1" value={form.max_participantes} onChange={(e) => setForm(f => ({ ...f, max_participantes: e.target.value }))}
                                placeholder={t('presenca.form.unlimited', 'Ilimitado')}
                                className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-100 focus:border-blue-400 outline-none" />
                        </div>
                        <div>
                            <label className="block text-xs font-medium text-gray-600 mb-1.5">{t('presenca.form.access_token_label', 'Token de Acesso')}</label>
                            <input type="text" value={form.token_acesso} onChange={(e) => setForm(f => ({ ...f, token_acesso: e.target.value }))}
                                placeholder={t('presenca.form.optional', 'Opcional')}
                                className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-100 focus:border-blue-400 outline-none" />
                        </div>
                    </div>
                    ...
                    <div className="flex items-center justify-end gap-3 pt-4 border-t border-gray-100">
                        ...
                        >
                            ...
                            {isSubmitting ? t('common.creating', 'Criando...') : t('presenca.form.submit', 'Criar Lista')}
                        </button>
                    </div>
                </form>
            </div>
