...
export default function ProcedimentosGeraisPage() {
    const [docs, setDocs] = useState<Document[]>([]);
    const [selectedDoc, setSelectedDoc] = useState<Document | null>(null);
    const [loading, setLoading] = useState(true);
    const { t } = useI18n();

    // Fetch documents from the API for Procedimentos Gerais category
    useEffect(() => {
        (async () => {
            try {
                setLoading(true);
                const res = await fetch('/api/documents?category=' + encodeURIComponent('Procedimentos Gerais'));
                if (res.ok) {
                ...
                console.warn('Falha ao carregar procedimentos gerais:', e);
            } finally {
                setLoading(false);
            }
        })();
    ...
    return (
        <MainLayout>
            <h1 className="text-3xl font-bold text-abz-text-black mb-6">
                {t('menu.procedimentosGerais', 'Procedimentos Gerais')}
            </h1>

            <div className="bg-white p-6 rounded-lg shadow-md">
                <h2 className="text-2xl font-semibold text-abz-text-black mb-6">
                    {t('procedimentos.mainDocument', 'Documentos')}
                </h2>
                ...
                {loading ? (
                    ...
                    <div className="flex flex-col items-center justify-center text-center text-gray-500 py-12 px-6 border-2 border-dashed border-gray-300 rounded-lg">
                        ...
                        <h3 className="text-xl font-semibold text-abz-text-black mb-2">
                            {t('procedimentos.noDocuments', 'Nenhum documento disponível')}
                        </h3>
                        <p className="text-sm">
                            {t('procedimentos.noDocumentsDescription', 'Os documentos de procedimentos gerais serão disponibilizados em breve.')}
                        </p>
                        <p className="text-xs text-gray-400 mt-4">
                            {t('procedimentos.adminNote', 'Administradores podem adicionar documentos em Admin > Documentos')}
                        </p>
                    </div>
                ) : (
                            ...
                            >
                                ...
                                <div className="flex items-center gap-2 mt-4">
                                    ...
                                    >
                                        ...
                                        {t('procedimentos.view', 'Visualizar')}
                                    </button>
                                    <a
                                        href={doc.file}
                                        download
                                    ...
                                    >
                                        ...
                                        {t('procedimentos.download', 'Download')}
                                    </a>
                                </div>
                            </div>
                        ))}
