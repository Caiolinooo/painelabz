    ...
    <div className="flex-1 p-6">
      <div className="max-w-5xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-gray-800">{t('eSocial.revisaoTitle')}</h1>
            <p className="text-sm text-gray-500">{t('eSocial.revisao.title')}</p>
          </div>
          <button
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-12 text-center">
            ...
            <p className="text-gray-500">{t('eSocial.revisao.noPending')}</p>
          </div>
        ) : (
          <div className="space-y-3">
            {eventos.map((evento) => (
              ...
              >
                ...
                <span className="inline-flex px-2.5 py-1 text-xs font-medium rounded-full bg-amber-100 text-amber-800">
                  {t('eSocial.eventStatus.pendingReview')}
                </span>
              </div>
            ))}
          </div>
