-- Drop table to recreate with correct types (CASCADE to drop indexes/dependents)
DROP TABLE IF EXISTS public.global_search_index CASCADE;

-- Create the global search index table
CREATE TABLE public.global_search_index (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    original_id TEXT, -- Changed to TEXT to support various ID types
...
DECLARE
    ...
    doc_url TEXT;
    doc_metadata JSONB;
    doc_original_id TEXT;
BEGIN
    doc_original_id := NEW.id::text; -- Cast ID to text
    ...
    ELSIF TG_TABLE_NAME = 'Reimbursement' THEN
        ...
        doc_content := COALESCE(NEW.descricao, '') || ' - ' || NEW.status;
    ...
    ELSE
        ...
        WHERE original_id = doc_original_id AND source_table = TG_TABLE_NAME;
            
        IF NOT FOUND THEN
            INSERT INTO public.global_search_index (original_id, source_table, title, content, url, metadata)
            VALUES (doc_original_id, TG_TABLE_NAME, doc_title, doc_content, doc_url, doc_metadata);
        END IF;
...
BEGIN
    ...
    INSERT INTO public.global_search_index (original_id, source_table, title, content, url, metadata)
    SELECT id::text, 'academy_courses', title, COALESCE(description, short_description), '/academy/course/' || id, jsonb_build_object('category_id', category_id)
    FROM public.academy_courses;
    
    INSERT INTO public.global_search_index (original_id, source_table, title, content, url, metadata)
    SELECT id::text, 'Reimbursement', 'Reembolso ' || protocolo, COALESCE(descricao, '') || ' - ' || status, '/reembolso/' || protocolo, jsonb_build_object('amount', COALESCE("valorTotal", valor_total), 'status', status)
