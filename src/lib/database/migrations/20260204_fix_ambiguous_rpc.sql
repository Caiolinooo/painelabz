-- FIX: Qualify all columns to avoid ambiguity between params/returns and table columns
...
DECLARE
    total_rows BIGINT;
BEGIN
    -- 1. Get total count for pagination (Qualified columns)
    SELECT COUNT(*) INTO total_rows
    FROM public.global_search_index gsi_count
    WHERE 
        gsi_count.search_vector @@ websearch_to_tsquery('portuguese', query_text)
        OR gsi_count.title ILIKE '%' || query_text || '%'
        OR gsi_count.content ILIKE '%' || query_text || '%';
