...
BEGIN
    ...
    ELSIF TG_TABLE_NAME = 'Reimbursement' THEN
        ...
        -- Coalesce to TEXT to avoid type mismatch
        doc_metadata := jsonb_build_object('amount', COALESCE(NEW."valorTotal", NEW.valor_total::text), 'status', NEW.status);
    END IF;
...
BEGIN
    ...
    INSERT INTO public.global_search_index (original_id, source_table, title, content, url, metadata)
    SELECT id::text, 'Reimbursement', 'Reembolso ' || protocolo, COALESCE(descricao, '') || ' - ' || status, '/reembolso/' || protocolo, jsonb_build_object('amount', COALESCE("valorTotal", valor_total::text), 'status', status)
