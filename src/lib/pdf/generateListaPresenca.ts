...
export async function generateListaPresencaPDF(
    ...
    doc.text('Hora / Time', colTime.x + 1, headerY);
    y += 7;
    ...
    for (let i = 0; i < participantes.length; i++) {
        ...
        if (y + rowHeight > 280) {
            ...
            doc.text('Hora / Time', colTime.x + 1, hy);
            y += 7;
            doc.setTextColor(51, 51, 51);
            doc.setFont('helvetica', 'normal');
        }
        ...
        // Time
        const signedTime = new Date(p.created_at).toLocaleTimeString(locale, { hour: '2-digit', minute: '2-digit' });
        ...
        if (p.assinatura_url) {
            try {
                ...
                doc.text('(erro / error)', colSignature.x + 4, textY);
                doc.setTextColor(51, 51, 51);
            }
        }
    ...
    doc.text(`Total de participantes / Total participants: ${participantes.length}`, margin, y);
    doc.text(`Gerado em / Generated on: ${new Date().toLocaleString('pt-BR')}`, pageWidth - margin, y, { align: 'right' });
    y += 4;
    doc.text('Documento gerado automaticamente pelo Portal ABZ Group / Document automatically generated by ABZ Group Portal — AN-QUA-001', pageWidth / 2, y, { align: 'center' });
