...
// Configuração do Exchange (principal)
...
export async function initEmailTransport() {
  ...
  try {
    // Configuração principal (Exchange)
    console.log('Inicializando transporte de email principal (Exchange)');
    ...
    if (process.env.EMAIL_SERVER) {
      ...
      try {
        ...
        console.log('Usando configuração manual:', {
          ...
          pass: exchangeConfig.auth.pass ? '****' : 'não definida'
        });

        transporter = nodemailer.createTransport(exchangeConfig);
      }
    } else {
...
export async function sendEmail(
  ...
  const mailOptions = {
    ...
    headers: {
      ...
      'List-Unsubscribe': '<mailto:apiabz@groupabz.com?subject=Unsubscribe>'
    },
