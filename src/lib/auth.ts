  ...
  try {
    ...
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <div style="***REMOVED*** #0066cc; padding: 20px; text-align: center;">
        ...
        <div style="padding: 20px; border: 1px solid #ddd; border-top: none;">
          ...
          <div style="text-align: center; margin: 30px 0;">
            <a href="${resetUrl}" style="***REMOVED*** #0066cc; color: white; padding: 12px 24px; text-decoration: none; border-radius: 4px; font-weight: bold;">Redefinir Senha</a>
          </div>
          <p>Se você não solicitou esta redefinição, ignore este email.</p>
          <p>Este link é válido por 1 hora.</p>
          <p>Atenciosamente,<br>Equipe ABZ Group</p>
        </div>
        <div style="***REMOVED*** #f5f5f5; padding: 15px; text-align: center; font-size: 12px; color: #666;">
...
export async function verifyPhoneLogin(phoneNumber: string, code: string, email?: string, inviteCode?: string): Promise<{ success: boolean; message: string; user?: User; token?: string; authStatus?: string; requiresPassword?: boolean; isNewUser?: boolean }> {
  ...
  try {
    ...
    if (!user) {
      ...
      if (!authCheck.authorized && phoneNumber !== ***REMOVED***) {
        // Se o status for pendente, informar que está aguardando aprovação
        if (authCheck.status === 'pending') {
          return {
            success: false,
