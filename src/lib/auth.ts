...
export function verifyToken(token: string): TokenPayload | null {
  try {
    ...
    if (token === supabaseServiceKey) {
      console.log('Token de serviço do Supabase detectado');
...
export async function initiatePhoneLogin(phoneNumber: string, email?: string, inviteCode?: string): Promise<{ success: boolean; message: string; hasPassword?: boolean; previewUrl?: string; method?: 'sms' | 'email'; authStatus?: string; authorized?: boolean; requiresPassword?: boolean }> {
  try {
    ...
    // Importar a função de envio de códigos
...
export async function verifyPhoneLogin(phoneNumber: string, code: string, email?: string, inviteCode?: string): Promise<{ success: boolean; message: string; user?: User; token?: string; authStatus?: string }> {
