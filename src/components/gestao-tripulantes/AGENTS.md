...
- **Viewport das páginas GT-family**: `GtPageShell` — coluna flex `flex-1 min-h-0` que preenche o `<main>` do MainLayout (`h-dvh` + `flex flex-col`). Header/abas/filtros/toolbar `shrink-0`. Em desktop (`lg:`), grade/tabela `flex-1 min-h-0 overflow-auto` (`GT_PAGE_SCROLLPORT_CLASS`); em mobile/telas compactas (`< 1024px`), `GtPageShell` usa `overflow-y-auto lg:overflow-hidden` e as tabelas mantêm altura mínima (`min-h-[320px]` / `min-h-[360px]`), garantindo que a página role com fluidez e elementos interativos permaneçam visíveis e clicáveis em qualquer resolução. Man Schedule usa `flush` (cancela o padding do `<main>`); o scrollport da grade continua `man-schedule-grid-classes.ts`.
...
- **Scroll Sincronizado do Man Schedule**:
  - Estilos em `globals.css` (`.man-schedule-scroll`) com barra visível de 14px e thumb `#94a3b8`.
  - Barra de rolagem horizontal superior sincronizada em `GTManScheduleTab` e `/department/man-schedule`, espelhando a posição da tabela e permitindo rolar a escala sem descer até o rodapé.
