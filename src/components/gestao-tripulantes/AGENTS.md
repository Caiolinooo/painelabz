...
- Aba **ASO Logística** na página GT: fila `solicitado` para ADMIN/MANAGER aprovar/reprovar com `useSignature` (sem segundo SignatureModal). Aprovação marca o ASO; reprovação exige motivo.
- **Fechamento de escalas / Envio DP**: admin `WorkflowFechamentoTab` lista gestores do portal (`availableManagers`; `first_name`/`last_name`/`email`/`role`). `ModalAprovacaoFechamento` (DP e Man Schedule) **sempre** `await requestSignature()` e envia `signature_url`; lista vazia de aprovadores = um ADMIN/MANAGER conclui; N nominados = 100% antes do e-mail. Não usar callback `onSign` (API do `SignatureRequest` é só `title`/`description`).
...
- Admin Fechamento: dropdown de aprovadores lista ADMIN/MANAGER reais (não placeholder duplicado). `npx tsx --test src/lib/gestao-tripulantes/fechamento-assinatura.test.ts`.
- GT aba ASO Logística lista solicitações do DP; aprovar com assinatura → status Marcado; reprovar mostra motivo no DP.
