...
- **Fechamento de escalas / Envio DP**: admin `WorkflowFechamentoTab` lista **qualquer usuário ativo** com e-mail (`listarUsuariosPortalAtivos`, `first_name`/`last_name`/`email`/`role`). Lista nominada = exatamente essas pessoas assinam, **independente do role**. Lista vazia = um ADMIN/MANAGER conclui. `ModalAprovacaoFechamento` **sempre** `await requestSignature()` e envia `signature_url`. Não usar callback `onSign`.
