...
export default function PurchaseOrderForm() {
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-8 pb-20">
            ...
            <div className="flex flex-col-reverse sm:flex-row justify-end gap-4 py-4">
                <button type="button" onClick={() => router.back()} className="w-full sm:w-auto px-6 py-3 sm:py-2 border rounded-lg hover:bg-gray-50 flex justify-center items-center font-medium">{t('purchaseOrders.form.actions.cancel')}</button>
                <button
                    type="submit"
                    disabled={uploading || (config?.max_value && totalValue > config.max_value)}
                    className="w-full sm:w-auto px-6 py-3 sm:py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 flex items-center justify-center gap-2 font-medium"
                >
