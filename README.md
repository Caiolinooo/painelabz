...
[![Version](https://img.shields.io/badge/Version-5.44.0-orange?style=for-the-badge)](#)
...
### Inteligencia Artificial [v5.44.0]
...
- **AI Companion**: global em todos os modulos; sessao STM segue o usuario (limpa so no logout); LTM Hermes-like (`ia_user_memory`); FAB pinwheel colorido
...
- Em **v5.44.0**, Companion global (sessão STM cross-módulo; limpa só no logout) + memória LTM Hermes-like (`ia_user_memory`) + FAB pinwheel colorido.- Tools Graph respeitam a solicitação do usuário (remetente, período, pasta, `limite=0` = máximo).
